...
def run_ai_simulation():
    ...
    while not game_over:
        ...
        if use_pruning:
            ...
            end_time = time.time()
            print(f"AI {current_player} plays column {col} | Nodes Pruned: {pruned_count} | Time taken: {end_time - start_time:.4f} seconds")
        else:
            start_time = time.time()
            col = get_minimax_move(board, depth=4)
            end_time = time.time()
            print(f"AI {current_player} plays column {col} | Time taken: {end_time - start_time:.4f} seconds")
